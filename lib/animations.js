    ...
    //DESCOMENTAR, VERSION FUNCIONAL
    // function playAnimation(targetEl) {
    //   const modelEl = targetEl.querySelector('[gltf-model]');
    //   if (!modelEl) return;
    
    //   // Evitar configurar animaciones si ya están activas
    //   if (modelEl.hasAttribute('animation-mixer')) return;
    
    //   const setupAnimation = () => {
    //     const animations = modelEl.getObject3D('mesh')?.animations || [];
    ...
    //     if ([0, 4, 13].includes(targetIndex)) {
    //       const idleClip = animations.find(anim => anim.name === 'Idle_2_HeadLow');
    //       animationName = idleClip ? 'Idle_2_HeadLow' : 'Idle_Headlow';
    //     }
